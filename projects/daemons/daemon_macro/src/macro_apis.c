...
{
  char buff[1024];
  snprintf(buff,sizeof(buff),"%s/%s",macro_LOG_FOLDER,namefile);
  *dstpath = strdup(buff);
...
static char *log_tempboard = NULL;
static char *log_feederboard = NULL;
static char *log_feederboard_01 = NULL;
...
#if (defined(DEBUG_ENABLE))
  if(!log_Init(macro_NAME_LOG_FEEDERBOARD,&log_feederboard,errmsg))
  {
    logger_mdebug( 0,logger_ERROR,"Error %s al intentar inicializar loger file: \"%s\"\n"\
                 , strerror(*errmsg),log_feederboard);
    return FALSE;
  }
  return TRUE;
#else
...
bool_t macro_FeederBoard_ProcessData(void * arg, int *errmsg)
{
  ...
  lret = log_WriteMsg( log_feederboard,(char*) ptm\
...
* @return valor del tipo int
*********************************************************************/
bool_t macro_FeederBoard_Init_01(void * arg, int *errmsg)
{
#if (defined(CROSS_COMPILER_ENABLE))
  /* mapeamos el periferico GPIO */
  if(!prpi_gpio_init(errmsg))
...
#if (defined(DEBUG_ENABLE))
  if(!log_Init(macro_NAME_LOG_FEEDERBOARD_01,&log_feederboard_01,errmsg))
  {
    logger_mdebug( 0,logger_ERROR,"Error %s al intentar inicializar loger file: \"%s\"\n"\
                 , strerror(*errmsg),log_feederboard_01);
    return FALSE;
  }
  return TRUE;
#else
  return log_Init(macro_NAME_LOG_FEEDERBOARD_01,&log_feederboard_01,errmsg);
#endif   
}
...
* @return valor del tipo int
*********************************************************************/
bool_t macro_FeederBoard_Deinit_01(int *errmsg)
{ 
    /*-- liberamos el path */
  if(log_feederboard_01 != NULL)
    free(log_feederboard_01);
#if (defined(CROSS_COMPILER_ENABLE))
  /*-- para otro uso del gpio */
  prpi_gpio_config(PIN_LED_SW0,1);
  prpi_gpio_deinit(errmsg/*pointer err-notify*/);
...
* @return valor del tipo int
*********************************************************************/
bool_t macro_FeederBoard_ProcessData_01(void * arg, int *errmsg)
{
  /* procesamos los datos arrivados */
  ...
  lret = log_WriteMsg( log_feederboard_01,(char*) ptm\
                     ,(LOG_WRITE_APPEND|LOG_WRITE_TIMESTAMP),errmsg);
  if(ptm != NULL)
    free(ptm);
...
bool_t macro_FeederBoard_Notify_01(void * arg, int *errmsg)
{  
  return TRUE;  
}
/* 
