...
{  
  ...
  if( argc <= 1)
    PERROR( FAILURE,"\tEl numero de Argumentos pasados son incorrectos %i\n"\
          , argc);    
  ...
  {  
    if(argument_FillList((const char*) argv[i],&arg_list,menu_opt) < 0 )
      PERROR(SUCCESS,"\tFalla al intentar Obtener los Argumentos Pasados.\n\t \"%s\"\n"\
            , strerror(EINVAL) );
    i++;
  }  
   ...
   */  
  DEBUG_CPRINTF(BLUE,"Parametros Obtenidos \n"\
   ...
   */
  if(!logger_init(&hdl_logger,1,arg_list.logger_path,NULL))
  {
  ...
  /* Realizamos el logger de los datos para armar el contexto
   */  
  logger_printf(0, logger_OK,"Parametros Obtenidos: \n"\
                 "\tdevice/library name: %s\n"\
                 "\tport \"%s\", baudrate %d\n"\
                 "\tEnumeracion Max Port %d, Reintentos de conexion %d\n"\
  ...
  do{  
    ...
    {
      logger_printf(0,logger_ERROR,"No se localizo el argumento -d|--device. finalizamos el daemon\n");
      break;
    }
  ...
  /*
    ...
    */
    ...
    {     
      logger_printf(0,logger_ERROR,"Argumento -d|--device Invalido {TempBoard|FeederBoard}.\n");
      break;      
    }
    logger_printf(0,logger_OK,"Librerias Cargadas para \"%s\"\n",arg_list.device);
  ...
  /*
    ...
    */
      /*
    snprintf(servicename,sizeof(servicename),"%s",arg_list.device);
    snprintf(portname,sizeof(portname),"%s",arg_list.port);*/
    ...
    logger_printf(0,logger_OK,"Daemon \"%s\" Iniciado.\n",arg_list.device/*servicename*/); 
  /*
    ...
    */
    ...
    if( (hdl_apis.fn_init != NULL) && ((hdl_apis.fn_init)(NULL,&errnumber) == FALSE))
    {
      ...
      logger_printf( 0,logger_ERROR,"Erro \"%s\" al intentar ejecutar fn_init() de la libreria\n"\
                   , strerror(errnumber));
      break;
    }      
    ...
    {
      logger_printf(0,logger_ERROR,"Dispositivo \"%s\" Desconectado, esperamos %d segundos.\n"\
    ...
    if(i >= arg_list.retryconn)
    {
      ...
      logger_printf(0,logger_ERROR,"Se agotaron los reintentos (%d). Salimos.\n",i);
      break;      
    }
    logger_printf(0, logger_OK,"Inicio exitoso para el Dispositivo \"%s\" "\
    ...
    {         
      ...
      logger_printf( 0,logger_WARNING,"Error \"%s\", Staus Error %d  llamado terminal_update().\n"\
      ...
      if(status == 1)
      {
        ...
        logger_printf(0,logger_ERROR,"Se Perdio comunicacion con el dispositivo,"\
                      "esperamos %d [seg].\n"\
                      , arg_list.timeoutdev );
        
        sleep(arg_list.timeoutdev);
        terminal_init(&arg_list/*portname,servicename*/,&hdl_port,&cfg_port);
      ...
      if(status == 2)
      {
        ...
        logger_printf(0,logger_ERROR,"Error \"%s\", Fallo la aplicacion que procesa los datos,"\
      ...
      logger_printf(0,logger_ERROR,"Se produjo un error desconocido.\n"\
  ...
  if(hdl_apis.fn_deinit != NULL)
  {
    if(!(hdl_apis.fn_deinit)(&errnumber))
      logger_printf(0,logger_ERROR,"Error \"%s\" Al ejecutar fn_deinit() de librerias.\n",strerror(errnumber));      
    else
      logger_printf(0,logger_OK,"Ejecucion sastifactoria de fn_deinit(), de la librerias.\n");
  }    
  
  logger_printf(0,logger_OK,"Daemon Finalizado.\n");  
  logger_deinit();  
...
static void daemonize( const char *idname )
{
  ...
  if(idname == NULL)
  {
    logger_printf(0,logger_ERROR,"No se crea el Daemon, call \"daemonize(NULL);\".\n");
    return;
  }
  if(strlen(idname) == 0)
  {
    logger_printf(0,logger_ERROR,"No se crea el Daemon, call \"daemonize(\"\"); Setring Vacio\".\n");
    return;
  }
  /* */
  logger_printf(0,logger_OK,"Se Inicia la creacion del Daemon: \"%s\".\n",idname);
  id = idname;
  pid_t pid;  
  ...
  if (pid < 0)
  {
    logger_printf( 0,logger_ERROR,"Error \"%s\", al intentar crear el primer child proccess.\n"\
                 , strerror(errno));
    exit(EXIT_FAILURE);
  }
  ...
  if (pid > 0)
  {
    logger_printf( 0,logger_OK,"Primer Proceso Hijo PID \"%d\", creado sastifactoriamente.\n"\
                 , pid);
    exit(EXIT_SUCCESS);
  }
  ...
  if( setsid() < 0)
  {
    logger_printf( 0,logger_ERROR,"Error \"%s\", al intentar crear una nueva session.\n"\
                 , strerror(errno));
    exit(EXIT_FAILURE);
  }
  ...
  if (pid < 0)
  {
    logger_printf( 0,logger_ERROR,"Error \"%s\", al intentar crear el segundo child proccess.\n"\
                 , strerror(errno));
    exit(EXIT_FAILURE);
  }    
  ...
  if (pid > 0)
  {
    logger_printf( 0,logger_OK,"Segundo Proceso Hijo PID \"%d\", creado sastifactoriamente.\n"\
                 , pid);
    exit(EXIT_SUCCESS);
  }
  ...
  /* Open the log file */
  logger_printf( 0,logger_OK,"Abrimos el syslog con el ID \"%s\".\n"\
                 , id);
                           ...
                           , serialterminal_cfg_t *cfgTerminal)
{
  ...
  do
  {
    ...
    if(stdcfile_CheckPathFile((const char*)path_port))
      break;    
#if (DAEMON_DEBUG == 1)    
    logger_printf(0,logger_ERROR,"No se encontro el Puerto %s\n",path_port);    
#endif     
  }while(++i < parglst->maxportnumber);
  
  if(i == MAX_PORT_NUMBERS)
  {
    logger_printf( 0,logger_ERROR,"No se encontro un Puerto Disponible, %d enumeraciones realizadas\n"\
                 , i );
    return FALSE;
  }
  ...
  if(serialterminal_OpenPort(hdlTerminal,(const char*)port,TRUE,&errnum ) == FALSE)
  {
    ...
    logger_printf( 0,logger_ERROR,"Error \"%s\" al intentar Abrir el puerto \"%s\"\n"\
  ...
  {
    syslog(ERROR_INSERT_SYSLOG,"%s",strerror(errnum));
    logger_printf( 0,logger_ERROR,"Error \"%s\" al intentar configurar el puert \"%s\"\n"\
                 , strerror(errnum),hdlTerminal->name);
    return FALSE; 
  }
  /**/
  ...
  /*-- enviamos el queryid al dispositivo conectado al puerto  */
  snprintf(boardTx,sizeof(boardTx),"%s",STRING_QUERY_ID);  
  logger_printf( 0,logger_INFO,"Enviando el String \"%s\", por el puerto\"%s\"\n"\
  ...
  {    
    ...
    logger_printf( 0,logger_INFO,"Se recibio el String \"%s\", desde el puerto \"%s\"\n"\
                 , boardRx,hdlTerminal->name);    
    
    if(strcmp(parglst->device/*devname*/,boardRx) == 0)
      return TRUE;      
    logger_printf( 0,logger_ERROR,"\tError Id Recibido: \n\t\"%s\"\n\t Se esperaba: \"%s\" \t reintentos: %d\n\n"\
                 , boardRx, parglst->device/*devname*/,retry);    
    stdctimer_msleep(725,NULL);
    
  }while(++retry < RETRY_GET_DEVICES_ID);
  
  
  logger_printf( 0,logger_ERROR,"Error con el Id Recibido \"%s\", reintentos %d agotados\n"\
               , boardRx, retry);
  return FALSE;   
}
                             ...
                             ,int *errMgs, uint8_t *errflag)
{
  ...
  /*logger_printf(0,logger_WARNING,"call stdcfile_CheckPathFile(%s).\n",hdlTerminal->name);*/
  ...
  if(strlen(buff) == 0)
  {
    ...
    logger_printf(0,logger_WARNING,"Se reinicio el Dispositivo., posible causa WDT\n");
    if(errflag != NULL)
      *errflag = 1;
    return FALSE;    
  }
    
  /*logger_printf(0,logger_WARNING,"Se recibio \"%s\".\n",buff);*/
  ...
  /*logger_printf(0,logger_WARNING,"call (hdlapis->fn_processdata)().\n");*/
  if(!(hdlapis->fn_processdata)((void*) buff,&errnum))
  {
    logger_printf(0,logger_WARNING,"Fallo el call (hdlapis->fn_processdata)().\n");
